...
public class Enemy1Attack extends Attack {
     ...
     *
     */
    private static final float SWEEP_ANGLE = 1.0f;
     ...
     */
    static {
        float x1 = (float) Math.cos(+ SWEEP_ANGLE/2);
        float y1 = (float) Math.sin(+ SWEEP_ANGLE/2);
        float x2 = (float) Math.cos(- SWEEP_ANGLE /2);
        float y2 = (float) Math.sin(- SWEEP_ANGLE/2);
     ...
     */
    public Enemy1Attack(float x, float y, int damage, float angleRadians, float length, long millis,BaseCharacter initializer){
